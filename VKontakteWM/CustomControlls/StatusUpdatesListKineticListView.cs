...
{
    class StatusUpdatesListKineticListView : KineticListView<StatusUpdatesListViewItem>
    {
        #region Members
        ...
        public StatusUpdatesListKineticListView()
        ...
        protected override void CalculateTextWidthsCustom(NativeItemData nativeItem, Graphics g, int textareaWidth, out int maxTextLines)
        {
            ...
            nativeItem.InfoLeftIndents[0] = UISettings.CalcPix(6);//иконка
            nativeItem.InfoLeftIndents[1] = UISettings.CalcPix(19);//Имя и статус
            nativeItem.InfoLeftIndents[2] = UISettings.CalcPix(10);//Дата, отступ справа

            nativeItem.InfoTopIndents[0] = UISettings.CalcPix(4);//иконка, дата и имя
        ...
        protected override void DrawGroupHeaderOn(Gdi gMem, int item, Rectangle rHeaderArea)
        {
            ...
            ////gMem.ExtTextOut(rHeaderArea.Right - UISettings.CalcPix(10) + UISettings.CalcPix(1), rHeaderArea.Top + Settings.GroupPixTopMargin * 2 + UISettings.CalcPix(1), GetItemAtIndex(item).Group, true);

            //gMem.TextColor = Settings.ListGroupTextColor;
            //gMem.ExtTextOut(rHeaderArea.Right - UISettings.CalcPix(10), rHeaderArea.Top + Settings.GroupPixTopMargin * 2, GetItemAtIndex(item).Group, true);

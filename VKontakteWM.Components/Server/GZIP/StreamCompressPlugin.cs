...
namespace Galssoft.VKontakteWM.Components.Server.GZIP
{
	public static class StreamCompressPlugin
	{
		...
		static void LoadBuiltInDecompression()
		{
			try
			{
				...
				{
					//FIXME
                    //CoreHelper.WriteLogEntry("Unable to find GZipStream or DeflateStream types!");
					m_Assembly = null;
				}
			}
			catch (Exception ex)
			{
				m_Assembly = null;
                //FIXME
                //CoreHelper.WriteLogEntry("Unable to load System.dll! Reason: " + ex.Message);
			}
		}
		...
		static void LoadSharpZipLibDecompression()
		{
			...
			try
			{
				...
				{
                    //FIXME
                    //CoreHelper.WriteLogEntry("Unable to find GZipInputStream or InflaterInputStream types!");
					m_Assembly = null;
				}
			}
			catch (Exception ex)
			{
				...
				//if (File.Exists(assemblyPath))
					//FIXME
                    //CoreHelper.WriteLogEntry("Unable to load SHarpZipLibAssembly! Reason: " + ex.Message);
			}
		}
		...
		/// <summary>
		/// Creates a new instance of GZip decompression stream
		/// </summary>
		...
		/// <summary>
		/// Creates a new instance of inflate stream
		/// </summary>
		/// <param name="input"></param>
		/// <returns></returns>
